            ...
            if (connection != null) {
                ...
                while (resultSet.next()) {
                    ...
                    auditform auditform = new auditform(
                        ...
                        resultSet.getString("picture") // Include the picture file name
                    );
...
<body>
    ...
    <form action="dashboard.jsp" method="get" style="text-align: right; margin-top: -40px; margin-right: 10px;">
    ...
    <% List<auditform> auditDataList = getAllAuditData();
       for (auditform auditform : auditDataList) { %>
        ...
        <div class="audit-box">
            ...
            <% if (auditform.getPicture() != null && !auditform.getPicture().isEmpty()) { %>
                <img src="<%= auditform.getPicture() %>" alt="Audit Picture" style="max-width: 100%;">
            <% } %>

            <!-- Form for updating findings -->
            <form action="updateFindings.jsp" method="post" style="display:inline;">
                <input type="hidden" name="auditId" value="<%= auditform.getId() %>">
                <button type="submit" class="approve-button">Update</button>
            </form>

            <form action="deleteFindings.jsp" method="post" style="display:inline;">
                <input type="hidden" name="auditId" value="<%= auditform.getId() %>">
                <button type="submit" class="ofi-ncr-button">Delete</button>
            </form>
        </div>
